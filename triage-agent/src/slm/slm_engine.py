...
class SLMEngine:
    def __init__(self, config: dict):
        ...
        # Fail fast with a clear message if Ollama isn't reachable / model isn't pulled
        try:
            ollama.list()
    ...
    def generate_stream(self, transcript: str, conversation_history=None, grammar=None):
        ...
        Note: `grammar` (GBNF) isn't supported by Ollama's API the way it is in
        raw llama.cpp. For Phase 2 structured slot-filling, use Ollama's
        `format="json"` parameter with a JSON schema instructed in the prompt
        instead -- see safety/ once that's built.
        """
        self._cancel_flag = False
...
if __name__ == "__main__":
    ...
    engine = SLMEngine(cfg["slm"])
    print("Response:")
    for chunk in engine.generate_stream("I've had a bad cough and fever for three days."):
